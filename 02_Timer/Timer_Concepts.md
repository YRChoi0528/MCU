...
타이머와 카운터는 **입력되는 클럭(펄스)의 개수를 세는 장치**이다.

### 1. 클럭(Clock)
- 일정한 시간 간격으로 `0`과 `1`이 반복되는 신호
- 모든 디지털 동작의 기준 시간을 제공

### 2. 타이머/카운터 장치
- 클럭이 한 번 들어올 때마다 내부 숫자를 1씩 증가(또는 감소)시키는 장치
...
- **ARR (Auto-Reload Register)**: 카운터 값이 ARR 값보다 커지게 되는 순간 카운터가 0이 되면서 오버플로우 발생
...
외부 신호의 물리적인 양을 측정하기 위해 두 번의 캡처 시점 차이를 이용
