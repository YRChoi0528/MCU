...
- **Down-count**: ARR → 0 → ARR …  
  ARR에서 시작하여 0까지 감소한 후, 다음 클럭에서 **언더플로우**와 함께 다시 ARR로 돌아가는 방식
...
PWM은 아래 비교 기능을 반복하여 일정한 주기를 가진 펄스를 만드는 것이다.
  ...
  3. 출력 로직 (Up-count 시):
     ...
     - `CNT ≥ CCR` **구간(신호 비활성화)** : 카운터가 임계값과 같아지거나 커지는 순간부터 주기(ARR)가 끝날 때까지는 Inactivate(비활성) 상태가 된다.
...
- 캡처된 CCR 값은 외부 이벤트가 발생한 **타임 스탬프**역할을 함

외부 신호의 물리적인 양을 측정하기 위해 두 번의 캡처 시점 차이($\Delta{T}$)를 이용
- **주기 및 주파수 측정** : 연속된 두 번의 상승 에지 상이의 CNT 차이를 계산
$$Period = (CCR_n-CCR_{n-1}) \times t_{CNT}$$

- **펄스 폭 측정** : 상승 에지에서 한 번, 하강 에지에서 한 번 캡처하여 그 사이의 시간을 측정 

> IC는 **외부에서 발생한 사건의 시간을 MCU 내부로 기록해오는 기능**이다.
