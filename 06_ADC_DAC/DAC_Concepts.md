...
0-255까지의 8bit 디지털 데이터를 한 번에 입력받아 0-5V 사이의 아날로그 신호로 변환한다.
...
- **하드웨어 특성**: `WR`신호가 `Low`상태일 때는 데이터 버스의 값이 전송되다가, `WR`신호가 `High`로 전환되는 상승 에지 시점에 데이터가 내부 레지스터에 확정되는 전형적인 래치 기반 인터페이스이다.
- **장단점**: 8개의 데이터 라인을 통해 한 번에 데이터를 전송하므로 속도가 매우 빠르지만, MCU의 GPIO 핀을 많이 소모한다는 단점이 있다.

### 4.2 직렬 DAC

실습에 사용되는 DAC081S101은 1개의 데이터 라인을 통해 시리얼 인터페이스로 제어되는 DAC이다. <br>
0-255까지의 디지털 데이터를 0-5V 아날로그 신호로 변환한다.

- **주요 핀 구성**:
  - `SYNC`: 데이터 프레임의 시작을 알리는 동기화 신호로, 통신 구간 동안 `Low`상태를 유지해야 한다.
  - `SCLK`: 데이터 입력을 동기화하기 위한 직렬 클럭 핀이다.
  - `DIN`: 클럭의 하강 에지에 맞춰 1bit씩 데이터가 전송되는 직렬 데이터 입력 핀이다.
...
- **장단점**: 단 3개의 핀(`SYNC`, `SCLK`, `DIN`)만으로 제어가 가능하여 공간과 핀 효율성이 좋지만, 병렬 방식에 비해 데이터를 전송하는 데 소요되는 시간이 길다.
